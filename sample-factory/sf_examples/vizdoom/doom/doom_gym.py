...
class VizdoomEnv(gym.Env):
    def __init__(
        ...
        self.curr_seed = 0
        self.rng = None
        # EVAL-ONLY: quando != None, reset() reaplica game.set_seed(este valor) no
        # jogo JÁ inicializado, antes de new_episode(). Necessário porque, no fluxo
        # de eval, o env persiste entre episódios e o engine Doom NÃO é resemeado
        # por reset(seed=) (seed() só atualiza self.curr_seed, não o engine) -> a
        # imagem-base do reset variava entre execuções (testado: max|Δ|≈127/255).
        # Com set_seed forçado, a obs do reset fica bit-idêntica. Permanece None no
        # treino -> comportamento byte-idêntico ao original.
        self._eval_doom_seed = None
        ...
        self.screen_w, self.screen_h, self.channels = 320, 240, 3
        self.screen_resolution = ScreenResolution.RES_320X240
    ...
    def set_eval_doom_seed(self, ep_seed: int):
        """Ativa o reseed determinístico do engine Doom para AVALIAÇÃO.

        Uso EXCLUSIVO de eval: o caller (scripts de eval) chama isto antes de cada
        episódio com ep_seed fixo. A partir daí, reset() RECRIA o jogo Doom
        (close + init) com essa semente, antes de new_episode().

        Por que recriar e não só set_seed: provado empiricamente que `game.set_seed`
        num jogo JÁ inicializado NÃO zera o RNG de simulação do ZDoom — episódios
        sucessivos no mesmo processo divergem (mesma seed/ações → trajetórias
        diferentes). Apenas `init()` re-semeia o RNG de simulação a partir do
        set_seed. Com close+init por episódio, a trajetória fica bit-idêntica (5/5).
        NÃO é chamado no treino -> treino byte-idêntico ao original (seed() continua
        só atualizando self.curr_seed; este caminho fica inativo com _eval_doom_seed=None)."""
        self._eval_doom_seed = int(ep_seed)

    def reset(self, **kwargs) -> Tuple[np.ndarray, Dict]:
        if "seed" in kwargs:
            self.seed(kwargs["seed"])

        # EVAL-ONLY: recria o jogo Doom com a semente do episódio, para zerar o RNG
        # de simulação do ZDoom (set_seed sozinho num jogo já vivo não basta — só
        # init() re-semeia). Garante trajetória reprodutível entre episódios no mesmo
        # processo. Inativo no treino (_eval_doom_seed is None) -> treino intacto.
        if self._eval_doom_seed is not None:
            self.curr_seed = self._eval_doom_seed
            if self.initialized and self.game is not None:
                self.game.close()
            self.initialized = False
