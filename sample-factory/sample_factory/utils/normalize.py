...
class ObservationNormalizer(nn.Module):
    ...
    def forward(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            # subtraction of mean and scaling is applied to image obs key.
            # For online DAE obs dict, key is "obs_noisy" instead of "obs".
            img_key = "obs_noisy" if "obs_noisy" in obs_clone else "obs"
            if self.should_sub_mean:
                obs_clone[img_key].sub_(self.sub_mean)

            if self.should_scale:
                obs_clone[img_key].mul_(1.0 / self.scale)
