...
class Learner(Configurable):
    ...
    def init(self) -> InitModelData:
        ...
        # --- CORREÇÃO: Inicialização do RND ---
        # 1. Usa a flag correta '--with_curiosity'
        # 2. Usa o nome correto 'self.curiosity_module'
        # 3. Passa os argumentos na ordem que 'rnd_module.py' espera: (cfg, obs_space, device)
        
        self.curiosity_module = None
        if getattr(self.cfg, 'with_curiosity', False):
            log.info(f"Habilitando módulo de curiosidade RND no dispositivo {self.device}...")
        ...
        # --- Online DAE optimizer ---
        self.ae_optimizer = None
        if getattr(self.cfg, "use_online_dae", False):
            dae_arch = getattr(self.cfg, "dae_arch", "shared")
            ae_lr = getattr(self.cfg, "ae_lr", 1e-4)
            ac = self.actor_critic
            if dae_arch == "dual" and ac.encoder_ae is not None:
                ae_params = list(ac.encoder_ae.parameters()) + list(ac.autoencoder_decoder.parameters())
            else:
                # Arch A: shared encoder — AE optimizer covers full encoder + decoder
                ae_params = list(ac.encoder.parameters()) + list(ac.autoencoder_decoder.parameters())
            self.ae_optimizer = torch.optim.Adam(ae_params, lr=ae_lr)
            log.info(f"Online DAE enabled (arch={dae_arch}, ae_lr={ae_lr}). ae_optimizer created.")

        # --- DAE debug metrics (observational; off by default) ---
        self._dae_debug = bool(getattr(self.cfg, "dae_debug_metrics", False)) and self.ae_optimizer is not None
        self._dae_drift_ref = None  # reference encoder features for drift measurement
        self._dae_drift_probe = None  # frozen probe batch (fixed input) for drift measurement
        if self._dae_debug:
            log.info("DAE debug metrics enabled (grad norms, cosine, representational drift).")
    ...
    def _load_state(self, checkpoint_dict, load_progress=True):
        ...
        # Checkpoints antigos não têm "curiosity" -> módulo segue da inicialização
        # (comportamento idêntico ao anterior). Novos restauram o RND completo.
    ...
    def _get_checkpoint_dict(self):
        ...
        # Estado do módulo de curiosidade (RND: predictor/target/otimizador/RMS).
        # Sem ele, o sinal intrínseco do treino é irrecuperável: a cada retomada
        # (curriculum) ou eval o módulo renasceria aleatório. Chave extra é
        # ignorada por código antigo (retrocompatível).
        ...
        for epoch in range(self.cfg.num_epochs):
            ...
            for batch_num in range(len(minibatches)):
                ...
                with timing.add_time("update"):
                    ...
                    # --- DAE debug: capture APPO gradient on the encoder before optimizer.step().
                    # Purely observational: clones .grad, never modifies it. The subsequent
                    # ae_optimizer.zero_grad() will clear these grads, so we snapshot them here.
                    enc_grad_appo = None
                    if self._dae_debug and self.ae_optimizer is not None:
                        enc_grad_appo = self._encoder_grad_vector()
                    # ---
                ...
                # --- Online DAE auxiliary step ---
                ae_loss_value = None
                ae_loss_mask_value = None
                ae_loss_bg_value = None
                enc_grad_norm_appo_value = None
                enc_grad_norm_ae_value = None
                enc_grad_cosine_value = None
                enc_feature_drift_value = None
                # Ablação de fase (C1): desliga o passo do DAE após o cutoff de env steps.
                # cutoff=0 -> sempre ativo (padrão). Não afeta C5/C6 (ae_optimizer is None).
                _dae_cutoff = int(getattr(self.cfg, "dae_phase_cutoff_steps", 0) or 0)
                _dae_active = _dae_cutoff <= 0 or self.env_steps < _dae_cutoff
                if self.ae_optimizer is not None and self.actor_critic.autoencoder_decoder is not None and _dae_active:
                    with timing.add_time("ae_step"):
                        self.ae_optimizer.zero_grad()
                        ac = self.actor_critic
                        # Arch B (dual): dedicated encoder, no gradient conflict with policy
                        # Arch A (shared): shared encoder — AE loss intentionally touches policy encoder
                        if ac.encoder_ae is not None:
                            z_ae = ac.encoder_ae({"obs": mb.normalized_obs["obs_noisy"]})
                        else:
                            z_ae = ac.encoder({"obs": mb.normalized_obs["obs_noisy"]})
                        reconstructed = ac.autoencoder_decoder(z_ae)
                        # obs_clean arrives already normalized by RunningMeanStd; divide by 255
                        # to bring it to the same [~0,1] scale the decoder outputs (Sigmoid).
                        target = mb.normalized_obs["obs_clean"].float() / 255.0
                        ae_loss_full = torch.nn.functional.mse_loss(reconstructed, target)

                        # Region-based diagnostics (no gradient).
                        # Derive mask_region from obs_clean in raw [0,255] scale.
                        # Raw obs is deleted after normalization (line ~1066), so we invert the
                        # RunningMeanStd transform: raw = norm * sqrt(var+eps) + mean.
                        # Pixels set to zero by the Chebyshev mask have raw value 0; we threshold
                        # at 10 (< 4% of [0,255]) to isolate them robustly.
                        with torch.no_grad():
                            _rms_dict = ac.obs_normalizer.running_mean_std
                            rms = _rms_dict.running_mean_std["obs_clean"] if (_rms_dict is not None and "obs_clean" in _rms_dict.running_mean_std) else None
                            if rms is not None:
                                _eps = 1e-8
                                _std = torch.sqrt(rms.running_var.float() + _eps)
                                _mean = rms.running_mean.float()
                                obs_clean_raw = mb.normalized_obs["obs_clean"].float() * _std + _mean
                            else:
                                # normalizer not active: obs_clean is already raw uint8 as float
                                obs_clean_raw = mb.normalized_obs["obs_clean"].float()
                            mask_region = (obs_clean_raw < 10.0).float()
                            bg_region = 1.0 - mask_region
                            n_mask = mask_region.sum().clamp(min=1)
                            n_bg = bg_region.sum().clamp(min=1)
                            ae_loss_mask_value = (((reconstructed.detach() - target) ** 2) * mask_region).sum() / n_mask
                            ae_loss_bg_value = (((reconstructed.detach() - target) ** 2) * bg_region).sum() / n_bg

                        ae_loss_full.backward()

                        # --- DAE debug: capture AE gradient on the encoder (now that .grad holds
                        # only the AE contribution, APPO grads were cleared by zero_grad above) and
                        # compare it against the APPO gradient snapshotted before optimizer.step().
                        if self._dae_debug:
                            with torch.no_grad():
                                enc_grad_ae = self._encoder_grad_vector()
                                if enc_grad_ae is not None:
                                    enc_grad_norm_ae_value = enc_grad_ae.norm(2).item()
                                if enc_grad_appo is not None:
                                    enc_grad_norm_appo_value = enc_grad_appo.norm(2).item()
                                if enc_grad_ae is not None and enc_grad_appo is not None:
                                    denom = enc_grad_ae.norm(2) * enc_grad_appo.norm(2)
                                    if denom > 1e-12:
                                        enc_grad_cosine_value = (
                                            torch.dot(enc_grad_ae, enc_grad_appo) / denom
                                        ).item()
                        # ---

                        self.ae_optimizer.step()
                        ae_loss_value = ae_loss_full.item()
                # ---------------------------------
                ...
                with torch.no_grad(), timing.add_time("after_optimizer"):
                    ...
                    if should_record_summaries:
                        # --- DAE debug: measure representational drift only when we are about to
                        # log, so it is always captured alongside the other summaries (fixed probe,
                        # no_grad, eval mode). Computed here to avoid the timing mismatch between an
                        # independent drift cadence and the summary cadence.
                        if self._dae_debug:
                            enc_feature_drift_value = self._measure_encoder_drift(mb)
                        # ---

                        # hacky way to collect all of the intermediate variables for summaries
                        summary_vars = {**locals(), **loss_summaries}

                        # --- RND INTEGRATION: Logs ---
                        if rnd_loss is not None:
                            summary_vars['rnd_loss'] = rnd_loss
                        # -----------------------------

                        # --- Online DAE Logs ---
                        if ae_loss_value is not None:
                            summary_vars['ae_loss'] = ae_loss_value
                        if ae_loss_mask_value is not None:
                            summary_vars['ae_loss_mask'] = ae_loss_mask_value.item()
                        if ae_loss_bg_value is not None:
                            summary_vars['ae_loss_bg'] = ae_loss_bg_value.item()
                        # -----------------------

                        # --- DAE debug Logs (observational only) ---
                        if enc_grad_norm_appo_value is not None:
                            summary_vars['dae_dbg_enc_grad_appo'] = enc_grad_norm_appo_value
                        if enc_grad_norm_ae_value is not None:
                            summary_vars['dae_dbg_enc_grad_ae'] = enc_grad_norm_ae_value
                        if enc_grad_cosine_value is not None:
                            summary_vars['dae_dbg_enc_grad_cosine'] = enc_grad_cosine_value
                        if enc_feature_drift_value is not None:
                            summary_vars['dae_dbg_enc_drift'] = enc_feature_drift_value
                        # -----------------------
    ...
    def _encoder_grad_vector(self):
        """Flatten the current .grad of the policy encoder into a single detached vector.
        Returns None if no gradient is present. Observational only — never modifies grads."""
        grads = [
            p.grad.detach().reshape(-1)
            for p in self.actor_critic.encoder.parameters()
    ...
    def _measure_encoder_drift(self, mb):
        """L2 distance between current encoder features on a FIXED probe batch and a
        reference snapshot taken at the first measurement. Freezing the probe input
        isolates weight drift from input variation. The first call caches the probe and
        the reference features and returns 0.0. Runs in eval mode under no_grad and
        restores the previous training mode afterwards."""
        obs_key = "obs_noisy" if "obs_noisy" in mb.normalized_obs else "obs"
        if self._dae_drift_probe is None:
            # Freeze a copy of the first probe batch; reused unchanged for all measurements.
            self._dae_drift_probe = mb.normalized_obs[obs_key].detach().clone()
        was_training = self.actor_critic.training
        self.actor_critic.eval()
        try:
            with torch.no_grad():
                feats = self.actor_critic.encoder({"obs": self._dae_drift_probe})
                feats = feats.detach().float().mean(dim=0)  # average over the batch dim
                if self._dae_drift_ref is None or self._dae_drift_ref.shape != feats.shape:
                    self._dae_drift_ref = feats.clone()
                    return 0.0
                return (feats - self._dae_drift_ref).norm(2).item()
        finally:
            if was_training:
                self.actor_critic.train()
    ...
    def _record_summaries(self, train_loop_vars) -> AttrDict:
        ...
        # --- Online DAE Logs ---
        if hasattr(var, 'ae_loss'):
            stats.ae_loss = var.ae_loss
        if hasattr(var, 'ae_loss_mask'):
            stats.ae_loss_mask = var.ae_loss_mask
        if hasattr(var, 'ae_loss_bg'):
            stats.ae_loss_bg = var.ae_loss_bg
        # -----------------------

        # --- DAE debug Logs ---
        for _k in ('dae_dbg_enc_grad_appo', 'dae_dbg_enc_grad_ae',
                   'dae_dbg_enc_grad_cosine', 'dae_dbg_enc_drift'):
            if hasattr(var, _k):
                stats[_k] = getattr(var, _k)
        # -----------------------
    ...
    def train(self, batch: TensorDict) -> Optional[Dict]:
        """
        Executa uma etapa de treinamento no Learner (GPU).
        
        ATUALIZAÇÃO TESE (RND Integration):
        Este método foi modificado para interceptar o batch de experiências e injetar
        manualmente as recompensas intrínsecas (curiosidade) diretamente no buffer
        de recompensas do ambiente.
        
        Lógica de Recompensa Total:
            R_total = (R_ambiente * rnd_ext_coef) + (R_curiosidade * intrinsic_reward_coeff)
            
        Se rnd_ext_coef for 0.0 (padrão da tese), o agente aprende EXCLUSIVAMENTE pela curiosidade.
        """
        with self.timing.add_time("misc"):
            self._maybe_update_cfg()
            self._maybe_load_policy()

        # --- RND INTEGRATION: Cálculo e Injeção ---
        ...
        if self.curiosity_module:
            with self.timing.add_time("curiosity_calc"):
                ...
                # When DAE online is active, obs is a dict {obs_noisy, obs_clean}.
                # RND expects {"obs": tensor} — route obs_noisy as the RND input.
                rnd_obs = batch["obs"]
                if isinstance(rnd_obs, dict) and "obs_noisy" in rnd_obs:
                    rnd_obs = {"obs": rnd_obs["obs_noisy"]}

                # 1. Calcula recompensa intrínseca (filtra obs terminais)
                raw_intrinsic_rewards = self.curiosity_module.calculate_rewards(rnd_obs, dones)

                # 2. Treina o predictor (filtra obs terminais, obs_rms em eval)
                rnd_loss = self.curiosity_module.update(rnd_obs, dones)

                # 3. Alinhamento temporal: obs tem T+1, rewards tem T
                ...
                # 4. Composição da recompensa final
        ...
        if num_invalids >= experience_size:
            ...
            with self.timing.add_time("train"):
                # Passamos o rnd_loss aqui para que ele seja logado no Tensorboard junto com as stats do PPO
            ...
            # --- ESTATÍSTICAS DE DEBUG DA TESE ---
            if raw_intrinsic_rewards is not None:
                # Adiciona a média da curiosidade (bruta) para aparecer no Tensorboard
                # Aparecerá na aba 'learner'
                stats["curiosity_raw"] = raw_intrinsic_rewards.mean().item()
                
                # Adiciona a recompensa total efetiva (aquela usada pelo PPO)
                stats["reward_total_mean"] = batch["rewards"].mean().item()
                
                # Diagnóstico no Console (confirmação visual de que está funcionando)
                if self.train_step % 200 == 0:
                    print(f"\n[DIAGNÓSTICO TESE] Step {self.train_step}")
                    print(f"  > Curiosidade Pura (Mean): {stats['curiosity_raw']:.5f}")
                    print(f"  > Recompensa Final (PPO):  {stats['reward_total_mean']:.5f}")
            # -----------------------------------------------
