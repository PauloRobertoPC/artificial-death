...
class ActorCriticSharedWeights(ActorCritic):
    def __init__(
        ...
        # When using online DAE, obs_space is a Dict with obs_noisy + obs_clean.
        # The encoder must see a {"obs": Box} space (VizdoomEncoder convention).
        # We remap obs_noisy → obs so encoder factories (VizdoomEncoder, MultiInputEncoder) work unchanged.
        self._use_online_dae = getattr(cfg, "use_online_dae", False)
        self._dae_arch = getattr(cfg, "dae_arch", "shared")
        if self._use_online_dae:
            encoder_obs_space = gym.spaces.Dict({"obs": obs_space["obs_noisy"]})
        else:
            encoder_obs_space = obs_space

        # in case of shared weights we're using only a single encoder and a single core
        self.encoder = model_factory.make_model_encoder_func(cfg, encoder_obs_space)
        ...
        # --- Online DAE auxiliary components ---
        if self._use_online_dae:
            enc_out = self.encoder.get_out_size()  # 512
            self.autoencoder_decoder = nn.Sequential(
                nn.Linear(enc_out, 64 * 7 * 7), nn.ReLU(),
                nn.Unflatten(1, (64, 7, 7)),
                nn.ConvTranspose2d(64, 64, 3, 1),          nn.ReLU(),
                nn.ConvTranspose2d(64, 32, 4, 2),          nn.ReLU(),
                nn.ConvTranspose2d(32,  1, 8, 4),          nn.Sigmoid(),
            )
            if self._dae_arch == "dual":
                # Arch B: dedicated AE encoder with independent weights (no gradient conflict)
                self.encoder_ae = model_factory.make_model_encoder_func(cfg, encoder_obs_space)
                self.encoders = [self.encoder, self.encoder_ae]
            else:
                self.encoder_ae = None
        else:
            self.autoencoder_decoder = None
            self.encoder_ae = None
    ...
    def forward_head(self, normalized_obs_dict: Dict[str, Tensor]) -> Tensor:
        if self._use_online_dae:
            # Route only obs_noisy to the policy encoder
            x = self.encoder({"obs": normalized_obs_dict["obs_noisy"]})
        else:
