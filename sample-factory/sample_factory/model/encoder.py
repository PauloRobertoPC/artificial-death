    ...
    def __init__(self, obs_shape: AttrDict, conv_filters: List, extra_mlp_layers: List[int], activation: nn.Module,
        ...
        for layer in conv_filters:
            if layer == "maxpool_2x2":
                ...
                if use_layernorm:
                    # GroupNorm(1, C) == LayerNorm sobre canais por amostra; estável no
                    # regime correlacionado do RL (não depende de estatísticas de batch)
                    # e independe da resolução espacial. Inserido antes da ativação.
                    conv_layers.append(nn.GroupNorm(1, out_ch))
...
class ConvEncoder(Encoder):
    def __init__(self, cfg: Config, obs_space: ObsSpace):
        ...
        use_layernorm = getattr(cfg, "encoder_use_layernorm", False)
        enc = ConvEncoderImpl(obs_space.shape, conv_filters, extra_mlp_layers, activation, use_layernorm)
